...
float lightOrigin[3];
float lightPosition[3] = { 1.0, -1.0, 3.0 };
...
void computeLightMatrix(void) {
	proj_light = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, 0.1f, 700.0f);
	mv_light = glm::lookAt(glm::vec3(lightPosition[0], lightPosition[1], lightPosition[2]), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0)) * glm::mat4(1);
}
...
void computeLightPosition(float* sight_vector) {
	float light_offset[3];
	/* first scale the sight vector by the eye_distance */
	/* we negate it because we want the offset vector from the eye_origin to the eye_pos */
	scaleVector(light_offset, sight_vector, -eye_distance);
	addVector(lightPosition, lightOrigin, light_offset);
